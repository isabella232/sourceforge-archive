...
/* ------------------------------------------------------------ */
/** Temporary implementation of FileHandler.
 *
 * While mostly functional, this handler is not implemented on
 * a generic handler model and has been included only to assist
 * with debugging and demonstrating the HTTP server.
 * @see
...
public class FileHandler
{
        ...
        throws HttpException, IOException
    {
        ...
        if (method.equals(HttpRequest.__GET) ||
            ...
            handleGet(request, response, uri, filename, endsWithSlash);        
            ...
            handleMove(request, response, uri, filename);
        else {
        ...
        throws IOException
    {
        ...
        if (file.exists())
        {
            ...
            if (file.isDirectory())
            {
                ...
                if (!indexSent)
                    sendDirectory(request,response,file,!("/".equals(uri)));
            }
        ...
        throws IOException
    {
        ...
        String newUri = request.getField("New-uri");
        if (newUri.indexOf("..")>=0)
        {
            response.sendError(response.__405_Method_Not_Allowed,
                               "File contains ..");
        ...
        try
        {    
            String newFilename = _fileBase+newUri;
