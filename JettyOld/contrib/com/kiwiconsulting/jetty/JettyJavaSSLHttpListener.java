 ...
 *<p>
 *
 * This implementation is trash.  I just wanted to get something
working.
 *
...
public class JettyJavaSSLHttpListener extends HttpListener
{
    ...
    public static final String DEFAULT_KEYSTORE  = System.getProperty(
"user.home" ) + File.separator + ".keystore";
...
() );
	...
	public JettyJavaSSLHttpListener( InetAddrPort p_address, HttpServer
p_server )
        throws IOException
	{
    ...
    public JettyJavaSSLHttpListener( InetAddrPort p_address, HttpServer
p_server,
                                     int p_minThreads, int
p_maxThreads, int p_maxIdleTimeMs)
        throws IOException
    {
...
p_maxIdleTimeMs );
		...
		System.err.println( "JettyJavaSSLHttpListener started on " +
p_address );
	}
    ...
    private SSLServerSocketFactory createFactoryHACK()
        throws Exception
    {
        char[] password = System.getProperty( PASSWORD_PROPERTY,
DEFAULT_PASSWORD ).toCharArray(); // hmm..
...
//          throws Exception
//      {
//          return SSLContext.getInstance( "SSL", "SunJSSE"
).getServerSocketFactory();
//      }

	protected ServerSocket newServerSocket( InetAddrPort p_address, int
p_acceptQueueSize )
        throws IOException
	{
        ...
        try
        {
            ...
            factory = createFactoryHACK();
                ...
                // yeah right...
                //factory = (SSLServerSocketFactory)
...
p_address.getPort(), p_acceptQueueSize, p_address.getInetAddress() );
        ...
        throws IOException
    {
		...
		try
		{
			...
			s.startHandshake();  // this should block until the handshaking is
done
            return (Socket) s;
		}
