...
import com.mortbay.Servlets.DispatchServlet;
import com.mortbay.Servlets.ServletDispatch;
...
public class PackageIndex extends DispatchServlet
{
	...
	Enumeration initE = config.getInitParameterNames();
	while (initE.hasMoreElements()){
	    String key = (String)initE.nextElement();
	    if (!key.equals(Page.PageType))
		sconfig.put(key, config.getInitParameter(key));
	}
